...
function countVowels(str) {
  ...
  arrVowels.forEach((vowels) => {
    const index = arrStr.indexOf(vowels);
    arrStr.forEach((el) => {
      if (el === vowels) arrResult.push(arrStr[index]);
    });
  });
...
function isPalindrome(str) {
  const strStandart = str
    .toLowerCase()
    .replaceAll(/\p{P}/gu, '')
    .split(' ')
    .join('');
  const strRevers = strStandart.split('').reverse().join('');
  if (strStandart === strRevers) return true;
  return false;
}
